# The Official  Konstant Info Swift Style Guide.
...
Swift types are automatically namespaced by the module that contains them and you should not add a class prefix such as KIS. If two names from different modules collide you can disambiguate by prefixing the type name with the module name. However, only specify the module name when there is possibility for confusion which should be rare.
...
Where an Xcode project is involved, the organization should be set to `Konstant Info` and the Bundle Identifier set to `com.KonstantInfo.TutorialName` where `TutorialName` is the name of the tutorial project.
...
Smiley faces are a very prominent style feature of the [ Konstant Info](https://www.konstantinfo.com/) site! It is very important to have the correct smile signifying the immense amount of happiness and excitement for the coding topic. The closing square bracket `]` is used because it represents the largest smile able to be captured using ASCII art. A closing parenthesis `)` creates a half-hearted smile, and thus is not preferred.
